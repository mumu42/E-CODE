  ...
  if (!started) {
    return (
      <div className="container mx-auto px-4 py-12 max-w-3xl text-center">
        <h1 className="text-2xl font-bold mb-4">全真限时模考</h1>
        <p className="text-gray-600 dark:text-gray-300 mb-8">
          考试包含阅读、听力、写作、口语四个部分，限时 45 分钟，由 AI 自动评分。
        </p>
        <Button onClick={() => { setStarted(true); loadMaterials(); }} size="lg">
          开始考试
        </Button>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-3xl">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold">全真模考</h1>
        {!finished && (
          <ExamTimer
            seconds={TOTAL_SECONDS}
            onFinish={() => {
              alert("考试时间到，自动提交");
              if (step === "reading") handleReadingSubmit();
              else if (step === "listening") handleListeningSubmit();
              else if (step === "writing") handleWritingSubmit();
              else if (step === "speaking") handleSpeakingSubmit();
            }}
          />
        )}
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="bg-muted p-4 rounded-md space-y-2">
              ...
              {!isTTSSupported() && <p className="text-xs text-orange-600">当前浏览器不支持 TTS。</p>}
            </div>
            <div className="space-y-4">
      ...
      {step === "speaking" && speakingTopic && (
        <Card>
          <CardHeader>
            <CardTitle>第四部分：口语</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="bg-muted p-4 rounded-md text-sm">
              <p className="font-bold">场景：{speakingTopic.scenario}</p>
      ...
      {step === "result" && record && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="text-4xl font-bold text-primary">{record.score}分</div>
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-muted p-3 rounded-md">
                <p className="text-sm text-muted-foreground">阅读</p>
                <p className="text-xl font-bold">{record.sectionScores?.reading ?? 0}</p>
              </div>
              <div className="bg-muted p-3 rounded-md">
                <p className="text-sm text-muted-foreground">听力</p>
                <p className="text-xl font-bold">{record.sectionScores?.listening ?? 0}</p>
              </div>
              <div className="bg-muted p-3 rounded-md">
                <p className="text-sm text-muted-foreground">写作</p>
                <p className="text-xl font-bold">{record.sectionScores?.writing ?? 0}</p>
              </div>
              <div className="bg-muted p-3 rounded-md">
                <p className="text-sm text-muted-foreground">口语</p>
                <p className="text-xl font-bold">{record.sectionScores?.speaking ?? 0}</p>
              </div>
            </div>
            {writingFeedbackText && (
              <div className="bg-muted p-3 rounded-md text-sm">
                <p className="font-medium">写作反馈</p>
                <p>{writingFeedbackText}</p>
              </div>
            )}
            {speakingFeedbackText && (
              <div className="bg-muted p-3 rounded-md text-sm">
                <p className="font-medium">口语反馈</p>
                <p>{speakingFeedbackText}</p>
              </div>
            )}
            <div className="flex gap-2">
              <Button onClick={() => router.push("/exam/full")}>再来一次</Button>
